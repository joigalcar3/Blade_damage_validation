        ...
        for alpha_angle_counter, alpha_angle in enumerate(alpha_angle_lst):
            for wind_speed_counter, wind_speed in enumerate(wind_speed_lst):
                for rpm_counter, rpm in enumerate(rpm_lst):
                    ...
                    modelled_healthy_T = -np.mean(F_healthy_lst[-1, :])
                    modelled_healthy_T_amplitude = np.max(F_healthy_lst[-1, :])-np.min(F_healthy_lst[-1, :])
                    modelled_healthy_N = np.mean(M_healthy_lst[-1, :])
                    modelled_healthy_N_amplitude = np.max(M_healthy_lst[-1, :]) - np.min(M_healthy_lst[-1, :])

                    if blade_damage == 0:
                        data_row += [modelled_healthy_T, modelled_healthy_N]
                    else:
                        data_row += [modelled_healthy_T, modelled_healthy_T_amplitude,
                                     modelled_healthy_N, modelled_healthy_N_amplitude]
